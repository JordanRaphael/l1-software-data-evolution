...
public class AgglomerativeClusterExtractor implements ClusterExtractor {
	...
	public ClusterCollector extractAtMostKClusters(GlobalDataManager dataKeeper, int numClusters, Double birthWeight,
		...
		init(dataKeeper, initSolution);

		ClusterCollector currentSolution = new ClusterCollector();
		currentSolution = newClusterCollector(initSolution, birthWeight, deathWeight, changeWeight,
				dataKeeper.getAllPPLSchemas().size() - 1);
		while (currentSolution.getClusters().size() > numClusters) {
			currentSolution = newClusterCollector(currentSolution, birthWeight, deathWeight, changeWeight,
					dataKeeper.getAllPPLSchemas().size() - 1);
		}
	...
	public ClusterCollector newClusterCollector(ClusterCollector prevCollector, Double birthWeight, Double deathWeight,
			Double changeWeight, int dbDuration) {
		...
		for (int oldI = 0; oldI < oldClusters.size(); oldI++) {
			...
			for (int oldI1 = 0; oldI1 < oldClusters.size(); oldI1++) {
				...
				distances[oldI][oldI1] = currentCluster.distance(clusterToCompare, birthWeight, deathWeight,
						changeWeight, dbDuration);
			}
		}
	...
	public ClusterCollector init(GlobalDataManager dataKeeper, ClusterCollector clusterCollector) {
		...
		for (Map.Entry<String, PPLTable> pplTable : tables.entrySet()) {
			Cluster c = new Cluster(pplTable.getValue().getBirthVersionID(), pplTable.getValue().getDeath(),
					pplTable.getValue().getDeathVersionID(), pplTable.getValue().getDeath(),
					pplTable.getValue().getTotalChanges());
