...
public class TableConstructionZoomArea implements PldConstruction {
	...
	private TreeMap<String, PPLTable> allPPLTables = new TreeMap<String, PPLTable>();
	...
	public TableConstructionZoomArea(ArrayList<Phase> phases, TreeMap<Integer, PPLTransition> allPPLTransitions,
			TreeMap<String, PPLSchema> allPPLSchema, TreeMap<String, PPLTable> allPPLTables,
			ArrayList<String> sSelectedTables, int selectedColumn) {
		this.allPPLTransitions = allPPLTransitions;
		this.allPPLTables = allPPLTables;
		this.phases = phases;
		allPPLSchemas = allPPLSchema;
	...
	private void fillSelectedPPLTransitions() {
		...
		if (selectedColumn == 0) {
			pplTransitions = allPPLTransitions;
		} else {
			pplTransitions = phases.get(selectedColumn - 1).getPhasePPLTransitions();
	...
	private void fillSelectedPPLSchemas() {
		...
		for (Map.Entry<Integer, PPLTransition> pplTr : pplTransitions.entrySet()) {
			...
			selectedPPLSchemas.put(pplTr.getValue().getOldVersionName(),
					allPPLSchemas.get(pplTr.getValue().getOldVersionName()));
