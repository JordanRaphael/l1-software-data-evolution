...
public class Phase {
	...
	public void setStartPos(int startPos) {
		...
		startSQLFile = transitionHistory.getValues().get(startPos).getOldVersionFile();
	}

	public void setEndPos(int endPos) {
		this.endPos = endPos;
		endSQLFile = transitionHistory.getValues().get(endPos).getNewVersionFile();
	}
	...
	public String toStringShort() {
		...
		for (int i = startPos; i <= endPos; i++) {
			...
			int currUpdates = currTransition.getNumAttrDel() + currTransition.getNumAttrDelWithDelTables()
					+ currTransition.getNumAttrInKeyAlt() + currTransition.getNumAttrIns()
					+ currTransition.getNumAttrInsInNewTables() + currTransition.getNumAttrWithTypeAlt();
	...
	public double distance(Phase anotherPhase, float timeWeight, float changeWeight) {
		...
		double changeDistance = Math.abs(totalUpdates - anotherPhase.totalUpdates)
				/ ((double) transitionHistoryTotalUpdates);

		double timeDistance = 0;
		Phase subsequent, preceding;
		if (startPos > anotherPhase.endPos) {
			subsequent = this;
		...
		timeDistance = (((transitionHistory.getValues().get(subsequent.startPos).getTime()
				- transitionHistory.getValues().get(preceding.endPos).getTime()) / 86400))
				/ (transitionHistory.getTotalTime());
	 ...
	 * 
	 * @param nextPhase has to be the next phase that is to be merged with the
	 *                  current one
	 * @return
	 */
	...
	public void connectWithPPLTransitions(GlobalDataManager tmpGlobalDataKeeper) {
		...
		for (Map.Entry<Integer, PPLTransition> tr : allPPLTransitions.entrySet()) {
			...
			if (found) {
				phasePPLTransitions.put(tr.getKey(), tr.getValue());
