...
INSERT INTO `schema_version` (`version`, `installationdate`) VALUES ('0_5_0', NOW());
...
CREATE TABLE `t_file` (
    ...
    `attemptnr` MEDIUMINT NOT NULL DEFAULT 0,
...
CREATE TABLE `t_source_file` (
    ...
    `waitforsources` TINYINT(1) NOT NULL,
    PRIMARY KEY (`guid`, `siteid`),
    INDEX (`siteid`, `state`, `lookupattemptnr`)    
) ENGINE=InnoDB;
...
BEGIN
    ...
    DECLARE cursor_siteid VARCHAR(50);
...
-- Remove trigger present up to version 0.4.1
DROP TRIGGER IF EXISTS t_file_insert;//

-- Remove trigger present on early 0.4.0 release candidates
...
CREATE TRIGGER t_file_before_update BEFORE UPDATE ON t_file
    ...
    BEGIN
        ...
        IF OLD.counter>0 AND NEW.counter=0 THEN
            ...
            IF OLD.state=1 THEN
                UPDATE t_site_source SET counter=counter-1 WHERE siteid=OLD.siteid AND sourcesiteid=OLD.sourcesiteid AND sshare=OLD.sshare;
                SET NEW.state=0;
        ...
        ELSEIF OLD.counter>0 AND NEW.counter>0 THEN
            ...
            IF OLD.state!=1 AND NEW.state=1 THEN
                INSERT INTO t_site_source (siteid, sourcesiteid, sshare, counter) VALUES (NEW.siteid, NEW.sourcesiteid, NEW.sshare, 1) ON DUPLICATE KEY UPDATE counter=counter+1;
...
-- Remove event present up to version 0.4.1
