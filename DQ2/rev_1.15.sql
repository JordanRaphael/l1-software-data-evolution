...
CREATE TRIGGER t_file_counter AFTER UPDATE ON t_file
    ...
    BEGIN
        ...
        IF OLD.state!=3 AND NEW.state=3 THEN
            INSERT INTO t_active_tratnsfer (siteid, sourcesiteid, sshare, counter) VALUES (NEW.siteid, NEW.sourcesiteid, NEW.sshare, 1) ON DUPLICATE KEY UPDATE counter=counter+1;
        END IF;
        IF OLD.state=3 AND NEW.state!=3 THEN
...
CREATE TRIGGER t_source_file_updater AFTER UPDATE ON t_source_file
   ...
   BEGIN
       ...
       END IF;
       IF OLD.creationdate!=NEW.creationdate AND NEW.state=1 THEN
           UPDATE t_file SET state=1 WHERE guid=NEW.guid AND siteid=NEW.siteid AND state=0;
       END IF;
   END;//
