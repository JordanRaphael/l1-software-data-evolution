...
INSERT INTO `t_history` (`active`) VALUES (0);
...
CREATE TRIGGER t_file_after_update AFTER UPDATE ON t_file
    ...
    BEGIN
        ...
        END IF;
        -- History information
        SELECT active INTO history FROM t_history;
        IF history=1 THEN 
            IF OLD.state=1 AND NEW.state=2 THEN
                IF OLD.transferstatedate != NULL THEN
                    INSERT INTO t_history_file_transfer (starttransferdate, endtransferdate, toolid, fsize, dsn, share, siteid, sourcesiteid, success, report, creationdate) VALUES (OLD.transferstatedate, NOW(), NEW.toolid, NEW.fsize, NEW.dsn, NEW.share, NEW.siteid, NEW.sourcesiteid, 1, NEW.report, NOW());
                ELSE
                    INSERT INTO t_history_file_transfer (starttransferdate, endtransferdate, toolid, fsize, dsn, share, siteid, sourcesiteid, success, report, creationdate) VALUES (NEW.transfersubmitdate, NOW(), NEW.toolid, NEW.fsize, NEW.dsn, NEW.share, NEW.siteid, NEW.sourcesiteid, 1, NEW.report, NOW());
                END IF;
            ELSEIF OLD.state=1 AND NEW.state=3 THEN
                IF NEW.transferstatedate != NULL THEN
                    INSERT INTO t_history_file_transfer (starttransferdate, endtransferdate, toolid, fsize, dsn, share, siteid, sourcesiteid, success, report, creationdate) VALUES (OLD.transferstatedate, NOW(), NEW.toolid, NEW.fsize, NEW.dsn, NEW.share, NEW.siteid, NEW.sourcesiteid, 0, NEW.report, NOW());
                ELSE
                    INSERT INTO t_history_file_transfer (starttransferdate, endtransferdate, toolid, fsize, dsn, share, siteid, sourcesiteid, success, report, creationdate) VALUES (NEW.transfersubmitdate, NOW(), NEW.toolid, NEW.fsize, NEW.dsn, NEW.share, NEW.siteid, NEW.sourcesiteid, 0, NEW.report, NOW());
                END IF;            
