...
DROP TABLE IF EXISTS `t_subscription_file`;		# FK constraint
...
###############
# Event to clean tables from deprecated entries
###############

DELIMITER //

DROP EVENT IF EXISTS `event_clean_file`;//
CREATE EVENT `event_clean_file`
    ON SCHEDULE EVERY 30 MINUTE
    DO
    BEGIN
        DELETE FROM t_source_file
            USING t_file, t_source_file
            WHERE t_file.counter=0
            AND t_source_file.guid=t_file.guid;
