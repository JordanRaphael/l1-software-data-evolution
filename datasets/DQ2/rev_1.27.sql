...
CREATE TRIGGER t_file_before_update BEFORE UPDATE ON t_file
    FOR EACH ROW
    BEGIN
  	    -- File was cancelled but now it is wanted again so send it back to initial state.
        IF OLD.counter=0 AND NEW.counter>0 AND NEW.state<3 THEN
            SET NEW.state=0;
        END IF;
    END;//
...
CREATE TRIGGER t_file_delete AFTER DELETE ON t_file
    ...
    BEGIN
        IF OLD.state=1 THEN
            -- Cancelled file must decrease counter.
            UPDATE t_site_source SET counter=counter-1 WHERE siteid=OLD.siteid AND sourcesiteid=OLD.sourcesiteid;
        END IF;
        IF OLD.state=2 THEN
            -- Cancelled file must decrease counter.
...
CREATE PROCEDURE fill_fsize ()
BEGIN
    DECLARE done INT DEFAULT 0;
    DECLARE cursor_fsize BIGINT;
    DECLARE cursor_siteid VARCHAR(36);
    DECLARE cursor_file_hold CURSOR FOR SELECT fsize,siteid FROM t_file;
    DECLARE CONTINUE HANDLER FOR SQLSTATE '02000' SET done=1;
    OPEN cursor_file_hold;
    REPEAT
        FETCH cursor_file_hold INTO cursor_fsize, cursor_siteid;
        IF NOT done THEN
        IF cursor_fsize > 0 AND cursor_fsize < 10000000 THEN
            INSERT INTO t_fsize_distribution (siteid, fsize, counter) VALUES (cursor_siteid, '< 10MB', 1) ON DUPLICATE KEY UPDATE counter=counter+1;
        ELSEIF cursor_fsize >= 10000000 AND cursor_fsize < 20000000 THEN
            INSERT INTO t_fsize_distribution (siteid, fsize, counter) VALUES (cursor_siteid, '>= 10 MB AND < 20MB', 1) ON DUPLICATE KEY UPDATE counter=counter+1;
        ELSEIF cursor_fsize >= 20000000 AND cursor_fsize < 50000000 THEN
            INSERT INTO t_fsize_distribution (siteid, fsize, counter) VALUES (cursor_siteid, '>= 20 MB AND < 50MB', 1) ON DUPLICATE KEY UPDATE counter=counter+1;
        ELSEIF cursor_fsize >= 50000000 AND cursor_fsize < 100000000 THEN
            INSERT INTO t_fsize_distribution (siteid, fsize, counter) VALUES (cursor_siteid, '>= 50 MB AND < 100MB', 1) ON DUPLICATE KEY UPDATE counter=counter+1;
        ELSEIF cursor_fsize >= 100000000 AND cursor_fsize < 200000000 THEN
            INSERT INTO t_fsize_distribution (siteid, fsize, counter) VALUES (cursor_siteid, '>= 100 MB AND < 200MB', 1) ON DUPLICATE KEY UPDATE counter=counter+1;
        ELSEIF cursor_fsize >= 200000000 AND cursor_fsize < 500000000 THEN
            INSERT INTO t_fsize_distribution (siteid, fsize, counter) VALUES (cursor_siteid, '>= 200 MB AND < 500MB', 1) ON DUPLICATE KEY UPDATE counter=counter+1;
        ELSEIF cursor_fsize >= 500000000 AND cursor_fsize < 1000000000 THEN
            INSERT INTO t_fsize_distribution (siteid, fsize, counter) VALUES (cursor_siteid, '>= 500 MB AND < 1GB', 1) ON DUPLICATE KEY UPDATE counter=counter+1;
        ELSEIF cursor_fsize >= 1000000000 AND cursor_fsize < 2000000000 THEN
            INSERT INTO t_fsize_distribution (siteid, fsize, counter) VALUES (cursor_siteid, '>= 1 GB AND < 2GB', 1) ON DUPLICATE KEY UPDATE counter=counter+1;
        ELSEIF cursor_fsize >= 2000000000 THEN
            INSERT INTO t_fsize_distribution (siteid, fsize, counter) VALUES (cursor_siteid, '>= 2GB', 1) ON DUPLICATE KEY UPDATE counter=counter+1;
        END IF;
        END IF;
    UNTIL done END REPEAT;
    CLOSE cursor_file_hold;
...
CREATE TRIGGER t_subscription_file_delete AFTER DELETE ON t_subscription_file
    ...
    BEGIN
        ...
        SELECT COUNT(*) INTO counter FROM t_subscription_file WHERE guid=OLD.guid;
        IF counter=0 THEN
