...
CREATE TRIGGER t_file_counter_remove AFTER DELETE ON t_subscription_file
    FOR EACH ROW
    BEGIN
        DECLARE guidcounter INT;
        -- Decrease counter on t_file table
        UPDATE t_file SET counter=counter-1 WHERE guid=OLD.guid AND siteid=OLD.siteid;
        -- Wipe records from t_source_file if it is last entry of GUID for all sites
        SELECT SUM(counter) INTO guidcounter FROM t_file WHERE guid=OLD.guid;
        IF guidcounter=0 THEN
...
CREATE TRIGGER t_file_counter AFTER UPDATE ON t_file
    ...
    BEGIN
        ...
        END IF;
        -- File has been cancelled so decrease t_active_transfer counter if in transfer state
        IF NEW.counter=0 AND OLD.counter>0 AND OLD.state=3 AND NEW.state=3 THEN
	    	UPDATE t_active_transfer SET counter=counter-1 WHERE siteid=OLD.siteid AND sourcesiteid=OLD.sourcesiteid AND sshare=OLD.sshare;
        END IF;
    END;//
...
CREATE TRIGGER t_source_file_updater AFTER UPDATE ON t_source_file
    FOR EACH ROW
    BEGIN
        -- Source file not available so try to set state to IN_ALLOCATION on t_file
        IF NEW.state=2 THEN
            UPDATE t_file SET state=1 WHERE guid=NEW.guid AND siteid=NEW.siteid AND state=0;
        END IF;
        -- Reinsertion of non available source file so try to set state to IN_ALLOCATION on t_file
        IF OLD.creationdate!=NEW.creationdate AND NEW.state=2 THEN
            UPDATE t_file SET state=1 WHERE guid=NEW.guid AND siteid=NEW.siteid AND state=0;
        END IF;
        -- Source file available or copied so try to set state to AVAILABLE on t_file
        IF NEW.state IN (1, 5) THEN
            UPDATE t_file SET state=7 WHERE guid=NEW.guid AND siteid=NEW.siteid AND state=0;
        END IF;
        -- Reinsertion of available or copied source file so try to set state to AVAILABLE on t_file
        IF OLD.creationdate!=NEW.creationdate AND NEW.state IN (1, 5) THEN
            UPDATE t_file SET state=7 WHERE guid=NEW.guid AND siteid=NEW.siteid AND state=0;
        END IF;        
    END;//

DROP TRIGGER IF EXISTS t_subscription_file_remove;//
CREATE TRIGGER t_subscription_file_remove AFTER DELETE ON t_subscription
    FOR EACH ROW
    BEGIN
