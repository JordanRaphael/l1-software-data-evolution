...
CREATE TABLE `t_file` (
    ...
    INDEX (`requestid`),
    INDEX (`state`, `siteid`, `sourcesiteid`),    # Only useful while we have GROUP BY for getSourcesForFilesToSubmit
...
CREATE TABLE `t_source_file` (
    ...
    PRIMARY KEY (`guid`, `siteid`, `state`),
    UNIQUE (`guid`, `siteid`),
    INDEX (`state`, `siteid`)
) ENGINE=InnoDB
PARTITION BY LIST(`state`) (
    PARTITION state0 VALUES IN (0),
    PARTITION state1 VALUES IN (1),
    PARTITION state2 VALUES IN (2),
    PARTITION state3 VALUES IN (3),
    PARTITION state4 VALUES IN (4),
    PARTITION state5 VALUES IN (5)
);
...
DROP EVENT `event_release_subscription`;
CREATE EVENT `event_release_subscription`
 ON SCHEDULE EVERY 30 MINUTE
 DO UPDATE t_subscription SET locked=0 WHERE modificationdate < DATE_SUB(NOW(), INTERVAL 1 HOUR);

DROP EVENT `event_release_lookup_request`;
CREATE EVENT `event_release_lookup_request`
 ON SCHEDULE EVERY 30 MINUTE
 DO UPDATE t_lookup_request SET locked=0 WHERE modificationdate < DATE_SUB(NOW(), INTERVAL 1 HOUR);

DROP EVENT `event_release_transfer_request`;
CREATE EVENT `event_release_transfer_request`
 ON SCHEDULE EVERY 30 MINUTE
 DO UPDATE t_transfer_request SET locked=0 WHERE modificationdate < DATE_SUB(NOW(), INTERVAL 1 HOUR);
